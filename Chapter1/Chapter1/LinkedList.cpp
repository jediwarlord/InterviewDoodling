...
{ // get item from linked list...
	Item * NodeFinder;
	NodeFinder = this->Head->next; // pointing to the first node in the list...
	//check to see if this the node we want...

	//as long as NodeFinder is not null we traverse the list...
	while(NodeFinder)
	{
		if (Compare(Key, NodeFinder))
		{
			//there is a match... return this node.. otherwise keep going..
			return NodeFinder;
		}
