...
bool createnewNode(int _data, Tree** Node)
{
...
{
	//check to see if _rootnode is null
	if (*_RootNode == NULL)
	{
		createnewNode(_data, _RootNode);
		return true;
	}

	if (_data <= (*_RootNode)->element)
	{
		insertBinaryNode(_data, &(*_RootNode)->left);
	}
	else {
		insertBinaryNode(_data, &(*_RootNode)->right);
	}
...
Tree * delete_Tree(int value, Tree * t)
{
	...
	if (value < t->element)
	{
		t->left = delete_Tree(value, t->left);
	}
	else if (value > t->element)
	{
		t->right = delete_Tree(value, t->right);
	}
...
asciinode * build_ascii_tree_recursive(Tree * t)
{
	...
	sprintf_s(node->label, "%d", t->element);
